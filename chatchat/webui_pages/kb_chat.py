...
def kb_chat(api: ApiRequest):
    ...
    with st.sidebar:
        ...
        tabs = st.tabs(["RAG Configuration", "Session Settings"])
        with tabs[0]:
            ...
            history_len = st.number_input("Number of historical dialogue turns:", 0, 20, key="history_len")
            kb_top_k = st.number_input("Number of knowledge matches:", 1, 20, key="kb_top_k")
            ## Bge model may exceed 1
            score_threshold = st.slider("Knowledge match score threshold:", 0.0, 2.0, step=0.01, key="score_threshold")
            return_direct = st.checkbox("Return only the search results", key="return_direct")
            ...
            with placeholder.container():
                ...
                elif dialogue_mode == "File Dialogue":
        ...
        with tabs[1]:
            ...
            if cols[0].button("Create new", on_click=add_conv):
                ...
    ...
    chat_input_placeholder = "Please enter your message. Use Shift+Enter to insert a line break."
    ...
    if prompt:
        ...
        if dialogue_mode == "Knowledge Base Q&A":
            ...
            chat_box.ai_say([
                ...
                f"Querying the knowledge base `{selected_kb}` ...",
            ])
        elif dialogue_mode == "File Dialogue":
            if st.session_state.get("file_chat_id") is None:
                st.error("Please upload a file before starting the dialogue")
                st.stop()
            ...
            chat_box.ai_say([
                Markdown("...", in_expander=True, title="Knowledge Base Match Results", state="running", expanded=return_direct),
                f"Querying the file `{st.session_state.get('file_chat_id')}` ...",
            ])
        else:
    ...
    export_btn.download_button(
        ...
        file_name=f"{now:%Y-%m-%d %H.%M}_Conversation history.md",
        mime="text/markdown",
        use_container_width=True,
    )
