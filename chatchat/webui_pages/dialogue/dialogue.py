...
def dialogue_page(
    ...
    with st.sidebar:
        ...
        tab1, tab2 = st.tabs(["Tool Settings", "Session Settings"])
        ...
        with tab1:
            ...
            use_agent = st.checkbox(
                "Enable Agent", help="Please ensure the selected model has Agent capabilities", key="use_agent"
            )
            output_agent = st.checkbox("Show Agent process", key="output_agent")
            ...
            st.file_uploader("Upload image", ["bmp", "jpg", "jpeg", "png"],
    ...
    chat_input_placeholder = "Please enter the conversation content. Use Shift+Enter to create a new line."
            ...
            try:
                for d in client.chat.completions.create(**params):
                    ...
                    if d.status == AgentStatus.error:
                        ...
                        if not output_agent:
                            continue
                        chat_box.insert_msg("Interpreting tool output results...")
    ...
    export_btn.download_button(
        ...
        file_name=f"{now:%Y-%m-%d %H.%M}_Conversation_history.md",
        mime="text/markdown",
        use_container_width=True,
    )
