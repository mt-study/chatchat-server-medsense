...
def run_webui_admin(
    ...
    logger.info(f"Webui ADMIN MODEL_PLATFORMS: {Settings.model_settings.MODEL_PLATFORMS}")
    set_httpx_config()

    host = Settings.basic_settings.WEBUI_ADMIN_SERVER["host"]
    port = Settings.basic_settings.WEBUI_ADMIN_SERVER["port"]

    script_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "webui_admin.py")
    ...
    logging_conf = get_config_dict(
        "INFO",
        get_log_file(log_path=Settings.basic_settings.LOG_PATH, sub_dir=f"run_webui_admin_{get_timestamp_ms()}"),
        1024 * 1024 * 1024 * 3,
        1024 * 1024 * 1024 * 3,
    )
    logging.config.dictConfig(logging_conf)  # type: ignore
...
def dump_server_info(after_start=False, args=None):
    ...
    from chatchat.settings import Settings
    from chatchat.server.utils import api_address, webui_address,webui_admin_address
    ...
    if after_start:
        ...
        if args.webui_admin:
            print(f"    Chatchat WEBUI ADMIN Server: {webui_admin_address()}")
    print("=" * 30 + "Langchain-Chatchat Configuration" + "=" * 30)
    print("\n")
...
async def start_main_server(args):
    ...
    webui_admin_started = manager.Event()
    if args.webui_admin:
        process = Process(
            target=run_webui_admin,
            name=f"WEBUIADMIN Server",
            kwargs=dict(
                started_event=webui_admin_started,
                run_mode=run_mode,
            ),
            daemon=False,
        )
        processes["webui_admin"] = process
    ...
    try:
        ...
        if p := processes.get("webui_admin"):
            p.start()
            p.name = f"{p.name} ({p.pid})"
            # webui_admin_started.wait()  # 等待webui_admin.py启动完成
        ...
        #     p.start()
        #     p.name = f"{p.name} ({p.pid})"
        #     webui_admin_started.wait()  # 等待webui.py启动完成
...
@click.option(
    ...
    "webui_admin",
    is_flag=True,
    help="run webui_admin.py server",
)

def main(all, api, webui,webui_admin):
    class args:
        ...
    args.all = all
    args.api = api
    args.webui = webui
    args.webui_admin = webui_admin
